function fun_main_nodip_classical(Parameter, directions, FoldPath, offSets, Type)
...
parfor iter_num=1:Parameter.iter_nums  %iter_nums
    ...
    %     fprintf("Iter:%d Moudle 1st has completed......\n",iter_num);
    %     toc
    %% -------模块二           =>               处理数据
    ...
    [~] = extract_unit_data_nodip_classical(seismicdata,iter_num,FoldPath,offSets,Parameter,Type);
...
%% 函数主体
function [texture_data] = extract_unit_data_nodip_classical(seismicdata,iter_num,FoldPath,offSets,Parameter,Type)
%% 文件内部参数
    ...
    for k= k_for
        ...
        for i = 1:inlineNum
            for j = LayerxlineNum
                ...
                if M==m
                    ceilData=zeros(size(ceilData))+1;
                end
                if M~=m   
