function fun_main_withdip_auto(Parameter, directions, FoldPath, offSets, Type)
...
%     fprintf("Iter:%d Moudle 1st is starting......\n",iter_num);
    ...
    %                     [angle_norm] = extract_data_dip(time,xlineNum,nums,iter_fr,FoldPath.angle_normfilename);
   ...
   seismicdata = handle_data(seismicdata,Parameter,directions);
    ...
    [~] = extract_unit_data_withdip_auto(seismicdata,p,q,angle,iter_num,FoldPath,offSets,Parameter,Type);
...
%% 函数主体
function [texture_data] = extract_unit_data_withdip_auto(seismicdata,p,q,angle,iter_num,FoldPath,offSets,Parameter,Type)
%% 文件内部参数
    ...
    for k= k_for
        ...
        for i = 1:inlineNum
            for j = LayerxlineNum
                ...
                %% ceilData 数值全为0，给随机数避免后续计算报错
%                 ceilData=ceilData+rand(size(ceilData));
                %% 纹理属性计算单元
